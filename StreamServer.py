...
class StreamServer:
    def __init__(self, host, port, image_path=None, refresh_signal=None):
        self.host = host
        self.port = port
        self.image_path = image_path or "/tmp/too_many_streams2.jpg"
    ...
    def _image_updater_loop(self):
        ...
        while True:
            ...
            logger.info(f"Image updater woke up, signaled={signaled}")
            ...
            try:
                ...
                streams_changed = gen.get_active_streams()
                logger.info(f"get_active_streams() returned {streams_changed}, active={len(gen.active_streams)}")
                if streams_changed or signaled:
                    if gen.generate(force=signaled):
                        logger.info("Image updated, restarting FFmpeg stream.")
                        self._start_ffmpeg()
                    else:
                        logger.info("generate() returned False (no change)")
            except Exception as e:
    ...
    def start(self):
        ...
        class StreamHTTPHandler(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                logger.info("TMS HTTP client connected, triggering image refresh")
        ...
        self.httpd = ThreadingHTTPServer((self.host, self.port), StreamHTTPHandler)
        try:
    ...
    def stop(self):
        logger.info("Stopping TooManyStreams HTTP Server...")
